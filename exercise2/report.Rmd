...
A: We use the local regression model $\texttt{logprice} = \beta_0 + \beta_1 
...
A: The predicted logprice of a diamond weighing 1 carat is 
...
A: $\beta_1 = \beta_2 = 0$ and $K_{i0} = 1$ would result in KNN-regression,
...
A: AIC is a criterion for (linear) model selection that penalizes the training error
...
A: AIC estimates the test error by penalizing the training error. With
...
A: The code below performs model selection with `bestglm()` based on AIC. The 
...
A: We also calculate and print the MSE of the test set using the best model in the
...
A: In the following code we build a model matrix for the covariates
...
A: In the following code we fit a lasso regression to the diamond data with
...
A: The MSE of the model predictions on the test set is calculated and printed
...
A: Building a regression tree in a _greedy_ manner means choosing the splitting
...
A: Regression trees can handle both numerical and categorical covariates. At
...
A: We fit a full regression tree to the diamond data with `logprice` as the
...
A: The MSE of the predictions by the regression tree fited in Q13 is calculated
...
A: Regression trees can be very sensitive to small changes in the data. E.g. if the
...
A: Other than the parameter $m$ for the size of the random subset to be considered
...
A: Boosting works a little differently from bagging and random forests. First a
...
A: In the following code we fit a random forest to the diamond data with 
...
A: Below we make a variable importance plot from the random forest. The left
...
A: As discussed in Q19, the random forest performs best when it comes to 
...
<<<<<<< HEAD
=======
Euclidean distance is the distance between points in a space of two or three dimensions. The average linkage is the mean intercluster dissimiliarity. Which means that we use the average dissimiliarty between pairs of clusters. <span style="color:red">Mangler forklaring på hvordan clustere bygges (greedy måte, slår sammen clustere som øker average linkage minst)</span>
>>>>>>> Solve problem 1
...
There's 
=======
The next plot provides further insight into the importance of $\textrm{glu}$ and $\textrm{skin}$. There's a clear structure in the plot below, showing that the fraction of people with diabetes increases drastically as the values of $\textrm{skin}$ and $\textrm{glu}$ increase. <span style="color:red">geom_jitter legger på randomness, kanskje heller bruke geom_points?</span>
>>>>>>> Solve problem 1
