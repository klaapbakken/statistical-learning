...
We start off by making sure that the data is represented correctly, check for missing values and a summary of the numeric data. 
...
mtry_list <- seq(1, n_predictors)
ntrees_list <- seq(1, 20, 1)
min_n_list <- seq(1, 10, 1)

parameter_df <- cross_df(list(mtry = mtry_list,
                          trees = ntrees_list,
                          min_n = min_n_list))

model_list <- parameter_df %>%
  as.list() %>%
  pmap(
    function(
      mtry,
...
get_prediction_df <- function(model, folds, engine){
  prediction_df <- folds %>%
    mutate(fitted_model =
             map2(
               recipe,
...
cv_list <- model_list %>% map(get_prediction_df,
                              folds=folds,
                              engine="randomForest")

get_accuracy <- function(cv_df){
accuracy_df <- tibble(truth =
                        cv_df %>%
                        select(prediction) %>%
                        unnest() %>%
                        pull(.pred_class),
                      estimate =
                        ...
                        unnest() %>%
                        pull(diabetes))
return(accuracy(accuracy_df, truth, estimate))
}

accuracy_list <- cv_list %>% map(get_accuracy) %>% map(".estimate")

parameter_df <- parameter_df %>% mutate(accuracy = accuracy_list) %>% unnest()

parameter_df %>% filter(accuracy == max(accuracy))
```


```{r}

cost_list <- map(seq(2, -3, -1),  ~10^(.))
rbf_sigma_list <- map(seq(0, -4, -1),  ~10^(.))
parameter_df <- cross_df(list(cost = cost_list,
                          rbf_sigma = rbf_sigma_list))
