...
**Q20:** Finally, compare the results from c (subset selection), d (lasso), e (tree) and f (random forest): Which method has given the best performance on the test set and which method has given you the best insight into the relationship between the price and the covariates?
  ...
  **Q21:** What is the definition of a principal component score, and how is the score related to the eigenvectors of the matrix ${\hat {\bf R}}$. 
  ...
  The data is given in the matrix $\matr{X}$, of size $n \times p$, where the columns are points in space. The columns are then scaled and shifted to mean $0$ and variance $1$, and we then standardized version of the observations $\matr{Z}$, of size $n \times p$. From this standardized matrix we can estimate the gene-standardized covariance matrix $\matr{\hat{R}}$ of size $p \times p$ by the equation 
  $$
...
In Figure \ref{fig:firsteigvec} we can see the first eigenvector in the matrix $\matr{V}$ or what we found was equal to $\matr{\phi}4$. It is the eigenvector of the matrix $\matr{\hat{R}}$. It shows how to linear combine the $\matr{Z}$ into our new transformed space, the first \textit{principal component}, which is the component with the heigest variance. To find the number of eigenvectors we need to know the rank of the matrix $\matr{\hat{R}}$, which is $\mathrm{rank}(\matr{\hat{R}}) = min(p,n-1) = (6830,64-1) = 63$. But the \texttt{prcomp} returns 64 eigenvalue elements and the last element is for $\lambda = 0$, so $\matr{V}_{64} = \vect{0}$.
...
$$
To find at which number of \textit{principal components}, they explain $80\%$ of to total variance we need to write some code to calculate it. And this is done in the code below, and the result of which is $\textrm{# PC}``r res_i``. 
...
In Figure \ref{fig:pc1pc2} we see the first \textit{principal component} on the x-axis versus the second \textit{principal component} on the y-axis. The `K562` cells, that we know are part of the `LEUKEMIA` familiy, is clustered with exactly these cells for both versions \textit{A} and \textit{B}. `MCF7` cells is part of the `BREAST` cancer famility. In the figure it is clusterted together with the one of these cells, but also together with `COLON`Cells. The `BREAST`cancer cells doesn't really have any clustered effects in this plot, but there `COLON`cells are very clustered in the top right corner. In the top left corner there is a big cluster of many different cancer cells, `NSCLC`, `OVARIAN` and `RENAL` in particular. And within this cluster the `UNKnOWN`cells is placed, so this doens't really give a clear indication of which cancer familiy the `UNKNOWN` cells are part of. 
...
In these plots we can really see any clustered effect that could classy the cancer cells in any perticular way. 
...
$$
with $n_{C_k}$ being the number of points in the cluster number $k$, with the points having dimension $P$. The Euclidean distance is given by $||x_1- x_2||_2 = \sqrt{(x_1^2 + x_2^2)}$. Using this on the scaled observations $\matr{Z}$, one would end up with the partition shown in Figure \ref{fig:dendogram}.
...
From earlier questions we know that the principal components can be found by the equation $\vect{y}_k = \vect{z}_k \matr{V}$, and if we look at the dissimilarities in both the transformed space and in the scaled space, we arrive at the equation
\begin{equation}
\begin{array}{rcl}
||\vect{y}_1 - \vect{y}_2||_2 & = & ||(\vect{z}_1 - \vect{z}_2)^T \matr{V}||_2\\
 & \Downarrow & \\
||\vect{y}_1 - \vect{y}_2||_2 & \leq & ||\vect{z}_1 - \vect{z}_2||_2 ||\matr{V}||_2\\
 & \Downarrow & (||\matr{V}||_2 = 0)\\
||\vect{y}_! - \vect{y}_2||_2 & \leq & ||\vect{z}_1 - \vect{z_2}||_2
\end{array}.
\end{equation}
This tells us that there is a contraction in the transformation between the spaces. The principal components account for the correlation between the points aswell, and therefore estimation might prove more accurate.

In the figure below the result of running the hierarchical cluster algorithm with the principal components is shown. On the left side a dendogram of the partition is displayed, and in correspons to the names of the cells on the vertical axes to the right in the figure. Along the horistonal axes the principal components are displayed. The principal components and the cell types form a grid system in the main plot, and the color of which corresponds to the \textit{principal component scores}. Red color indicates high positive scores and blue colors indicate high negative scores while zero score is a light yellow or white color. We can by this knowledge say that there are more variances in the first few principal components and they become more similiar in the later principal components, with the last elements $64$ having $0$ variance, which corresponds to the points having the same color. This what we concluded earlier in the exercise, only explained visually. 
```{r}
library(pheatmap)
npcs=64 
...
Our first method we want to explore is logistic regression. Logstic regression assumes that the response takes on values in $\{0, 1\}$. In addition, we assume that $Y_i, i = 1,\ldots, n$ is Bernoulli - distributed with sucess probability $p_i$. The logistic function has the form
...
```
From the estimated values $\mathbf{\beta}$ shown above we can note that increased values for either of the covariates lead to higher values of $p_i$. It also worth noting that not all of the included covariates are significant. It's clear from the estimates and the related standard errors that $\textrm{bmi}$ and $\textrm{glu}$, which was identified as important during the exploratory phase, are important. It's worth noting that $\textrm{skin}$ is excluded, but this might make sense seeing as it was highly correlated with $\textrm{bmi}$. 
...
We proceed by fitting a random forest to our data. A random forest consists $B$ decision trees. The decision tree has parameters determing how many samples there needs to be in a node before we can split further. This is a tuning parameter. Only $m, m < p$ randomly chosen covariates can be considered at each split, and this is also a tuning parameter. When splitting a node we want to minimize the Gini index, given by
...
The parameters are found through the use of K - fold cross validation, with $K = 5$. The parameter search is by no means exhaustive, but the should be sufficient for our purpose. The number of trees is fixed at $B = 100$, seeing as this is not a tuning parameter and it's common to simple set to be "high enough". 


By taking a look at the mean decrease in the Gini index for the diffiferent covariates, we observe that $\textrm{bmi}$ and $\textrm{glu}$ are, again, very important.
...
rf_estimator <- rand_forest(mode = "classification",
  ...
  set_engine("randomForest") %>%
  fit(formula = formula(scaled_prepped),
      data=train_unscaled)

rf_estimator$fit$importance
```
...
results <- tibble(
  estimated_class = rf_estimator %>%
    predict(new_data=test_unscaled,
            type="class") %>%
    pull(.pred_class),
...
The hyperplane is defined by $L(\mathbf{x} \mid \mathbf{\beta}) = \beta_0 + \sum_{i=1}^n\beta_ix_i = 0$. By calculating $\textrm{sign}\left(L(\mathbf{x} \mid \mathbf{\beta})\mid_{\mathbf{x}=\mathbf{x}_{i}}\right)$ we obtain the predict class, given that classes are encoded as $Y \in \{-1, 1\}$.

In other words, the observations are classified by looking at which side of the hyperplane the observation is located. 
...
model %>% evaluate(test_scaled %>% select(-diabetes) %>% as.matrix(),
