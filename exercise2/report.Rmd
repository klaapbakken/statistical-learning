...
best_accuracy <- rf_param_df %>% filter(accuracy == max(accuracy)) %>% select(accuracy)
...
dt_param_df <- cross_df(
  list(
     cost_complexity = map(seq(-2, -5, -1),  ~10^(.)),
    tree_depth = seq(1, 1, 20),
    min_n = seq(1, 10, 1)
  )
)

model_list <- dt_param_df %>%
  as.list() %>%
  pmap(
    function(
      cost_complexity,
...
dt_param_df <- dt_param_df %>%
  mutate(accuracy = cv_list %>%
           map_dbl(validation_score))

dt_param_df %>% filter(accuracy == max(accuracy))

best_accuracy <- dt_param_df %>% filter(accuracy == max(accuracy)) %>% select(accuracy)

best_params <- dt_param_df %>%
  mutate(index = row_number()) %>%
  filter(accuracy == max(accuracy)) %>%
