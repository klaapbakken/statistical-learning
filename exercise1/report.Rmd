...
* `Residual standard error` is an unbiased estimator for the standard deviation
  of the error term $\epsilon$ in our model $y_i = \beta^\intercal x_i +
...
* `F-statistic` is a statistic for the null hypothesis 
  ...
  $H_0$ is clearly rejected. The value and the p-value is given by
  $$F = \frac{(TSS-RSS)/p}{RSS/(n-p-1)} \, , \quad \text{where} \quad
...
smaller than before (77.54%). Also, the standardized residuals do not seem to
...
a more parsimonous model. But all in all `modelA` is preferable, since our
model assumptions seem more reasonable for `log(FEV)` as response.

**Q6:** 
We wish to test $H_0: \beta_{Age} = 0$ against $H_1: \beta_{Age} \neq 0$. We
use the test statistic
$$T_{Age} = \frac{\hat{\beta}_{Age}}{\sqrt{c_jj}\hat{\sigma}} \sim t_{n-p-1} \,
,$$
where $c_jj$ is the second diagonal element of $X^\intercal X)^{-1}$. The
...
n = nrow(lungcap)
t = dt(alpha/2, n-p-1)
X = matrix(c(rep(1,n), lungcap[["Age"]], lungcap[["Htcm"]],
             lungcap[["Gender"]], lungcap[["Smoke"]]), nrow=n, ncol=p+1)
c = solve(t(X)%*%X)[2,2]
sigma_hat = summary(modelA)$sigma
CI99 = c(beta_age - t*sqrt(c)*sigma_hat, beta_age + t*sqrt(c)*sigma_hat)
CI99
```

A 99% confidence interval for $\beta_{Age}$ is found and printed about, using
$$ P(\hat{\beta}_j-t_{\alpha/2,n-p-1}\sqrt{c_{jj}}\hat{\sigma}
\le \beta_j \le \hat{\beta}_j+t_{\alpha/2,n-p-1}\sqrt{c_{jj}}\hat{\sigma})=
1-\alpha \, .$$
This interval tells us that when repeating the experiment and measuring the
