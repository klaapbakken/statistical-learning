...
1-\frac{1}{K}+\sum\limi0ts_{i\in N} y_i, \enspace j = 0
\end{cases}
\end{equation}
**Q10:** 
```{r,eval=TRUE}
...
**Q13:** 
The strategy used in the first line is the \textit{one standard error rule}, which means that we choose the highest K for which or $K = \max\limits_K\{CV(K) \leq CV(\hat{K}) + SE(\hat{K})\}$, where $\hat{K}$ is k.min from Q11.
```{r,eval=FALSE}
...
```
The ROC curve displays the relationship between the percentage of correctly predicted *negatives*, *specificity*, and the percentage of correcly predicted *positives*, *sensitivity*. Their respective equations are
$$
\textrm{specificity} =  \frac{\textrm{#true negatives}}{\textrm{#negatives}} = \frac{TN}{N} \in \ {0,1\},
$$
$$
\textrm{sensitivity} =  \frac{\textrm{#true positives}}{\textrm{#positives}} = \frac{TP}{P} \in \ {0,1\}.
$$
The $x$-axis being the specificity is flipped so that specificity $= 1$ is to the left in the ROC. The best scenario is if we don't have any missclassifications, then the ROC curve would be in the top left corner, and the *"area under the curve"*(AUC) would be 1. We had AUC $=$ `r myRoc$auc`.

The diagonal line in the figure is $specificity = sensitivity$ and it represent a predictor of random guessing. It would be the same as sampling from a $u\sim \mathcal{U}[0,1]$. If we draw a positive class 50% of the samples, it is expected to draw negatives 50% aswell, and we would have $\textrm{sensitivity = 0.5} and \textrm{specificity = 0.5}. If we instead draw 30% the positive class, it would be expected to draw the negative class 30% of the time, leading to the ROC point (0.3, 0.3). Doing this over and over, would give us the *random guess line*.
**Q15:**
```{r,eval=TRUE}
...
```
```{r, eval=TRUE}
confusionMatrix(table(quality_score$y_hat[-tr],quality_score$y[-tr]))
confusionMatrix(table(KNNclass,quality_score$y[-tr]))
