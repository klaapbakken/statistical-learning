...
test.e = numeric(30)
for (k in ks){
  model1 <- class::knn(train = train.var, test = test.var, cl = train.def, k = k)
  model2 <- class::knn(train = train.var, test = train.var, cl = train.def, k = k)
  train.e[k] = sum(train.def!= model2)/nrow(train.var)
  test.e[k] = sum(test.def != model1)/nrow(test.var)
}
min(train.e)
which.min(train.e)
min(test.e)
